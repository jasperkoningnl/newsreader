...
export default function BronnenPage() {
  ...
  return <div className="mx-auto max-w-7xl px-5 py-8 md:px-10 md:py-12">
    ...
    <div className="mt-8 grid gap-6 md:grid-cols-2">{Object.entries(grouped).sort(([a],[b])=>a.localeCompare(b)).map(([cat,items])=><section key={cat} className="rounded-3xl border border-white/10 bg-black/40 overflow-hidden"><div className="border-b border-white/10 p-5"><h2 className="text-4xl font-semibold capitalize tracking-[-0.02em]">{cat}</h2></div><ul className="divide-y divide-white/10">{items.map(source=><li key={source.id} className="p-4"><div className="flex flex-col gap-3 md:flex-row md:items-center md:gap-4"><div className="min-w-0 flex-1"><p className="text-2xl leading-tight break-words">{source.name}</p><p className="truncate text-white/50">{source.url}</p></div><div className="flex flex-wrap items-center gap-2 md:justify-end"><button onClick={()=>handleToggle(source)} className={`h-8 w-14 shrink-0 rounded-full border ${source.active?"bg-white border-white":"border-white/20"}`}><span className={`block h-6 w-6 rounded-full bg-black transition-transform ${source.active?"translate-x-6":"translate-x-1"}`} /></button><select value={source.category ?? "other"} onChange={(e)=>handleCategoryChange(source, e.target.value)} className="h-9 min-w-[8rem] rounded-full border border-white/20 bg-transparent px-3 text-xs md:min-w-[9rem]">{categories.map(c=><option key={c} value={c} className="bg-black">{c}</option>)}</select><button onClick={() => handleDelete(source)} className="h-9 shrink-0 rounded-full border border-red-400/40 px-4 text-xs text-red-300 hover:bg-red-500/10">Verwijderen</button></div></div></li>)}</ul></section>)}</div>
