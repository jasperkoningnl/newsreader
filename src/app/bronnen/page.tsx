...
const CATEGORIES = ["tech", "news", "series", "sports", "games", "science", "culture", "philosophy", "other"];
...
export default function BronnenPage() {
  ...
  async function handleAdd(e: React.FormEvent) { e.preventDefault(); if (!urlInput || !nameInput) return; setAdding(true); setError(null); try { const res = await fetch("/api/sources", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ url: urlInput, name: nameInput, feed_url: feedUrlInput || null, category: categoryInput }) }); if (!res.ok) throw new Error("Failed to add source"); const created = await res.json(); setSources((prev) => [...prev, created]); setUrlInput(""); setNameInput(""); setFeedUrlInput(""); setCategoryInput("tech"); setDiscoverState("idle"); urlRef.current?.focus(); } catch (err) { setError(err instanceof Error ? err.message : "Unknown error"); } finally { setAdding(false); } }
